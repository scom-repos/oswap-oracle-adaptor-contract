...
import Bin from "./OSWAP_OracleChainlinkPriceGuardBinanceTestnet.json";

export class OSWAP_OracleChainlinkPriceGuardBinanceTestnet extends Contract{
    constructor(wallet: IWallet, address?: string){
        super(wallet, address, Bin.abi, Bin.bytecode);
        this.assign()
    }
    deploy(params:{wbnb:string,dai:string,factory:string,maxValue:number|BigNumber,deviation:number|BigNumber,returnAmmPrice:boolean}): Promise<string>{
        return this._deploy(params.wbnb,params.dai,params.factory,Utils.toString(params.maxValue),Utils.toString(params.deviation),params.returnAmmPrice);
    }
