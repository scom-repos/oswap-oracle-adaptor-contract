...
import Bin from "./OSWAP_OracleChainlinkPriceGuardFiatAvalancheTestnet.json";

export class OSWAP_OracleChainlinkPriceGuardFiatAvalancheTestnet extends Contract{
    constructor(wallet: IWallet, address?: string){
        super(wallet, address, Bin.abi, Bin.bytecode);
        this.assign()
    }
    deploy(params:{wavax:string,usdt:string,factory:string,maxValue:number|BigNumber,deviation:number|BigNumber,returnAmmPrice:boolean}): Promise<string>{
        return this._deploy(params.wavax,params.usdt,params.factory,Utils.toString(params.maxValue),Utils.toString(params.deviation),params.returnAmmPrice);
    }
